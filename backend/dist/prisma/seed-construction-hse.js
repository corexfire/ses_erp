...
exports.seedHse = seedHse;
async function seedHse(prisma, tenantId) {
    console.log('👷 Seeding HSE (Safety) Records...');
    const projects = await prisma.project.findMany({ where: { tenantId } });
    if (projects.length === 0)
        return console.log('⚠️ No projects found for seeding HSE.');
    ...
    const incidents = [
        ...
        },
        {
            ...
            incidentDate: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000),
            status: 'CLOSED',
...
//# sourceMappingURL=seed-construction-hse.js.map