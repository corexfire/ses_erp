...
exports.seedConstruction = seedConstruction;
async function seedConstruction(prisma, tenantId) {
    ...
    const managerRecords = await Promise.all(managers.map(m => prisma.employee.upsert({
        where: { tenantId_employeeNo: { tenantId, employeeNo: m.employeeNo } },
        update: m,
        create: { ...m, tenantId, email: `${m.employeeNo.toLowerCase()}@ses-erp.local` }
    })));
    const hseRecords = await Promise.all(hseOfficers.map(h => prisma.employee.upsert({
...
//# sourceMappingURL=seed-construction.js.map