...
async function main() {
    ...
    for (let i = 0; i < 40; i++) {
        ...
        const expectedVal = Math.floor(Math.random() * 500) * 1000000 + 10000000;
    ...
    for (let i = 0; i < 30; i++) {
        ...
        await prisma.serviceTicket.upsert({
            where: { tenantId_code: { tenantId: tenant.id, code } },
            update: { status: ticketStatuses[Math.floor(Math.random() * ticketStatuses.length)] },
            create: {
                tenantId: tenant.id,
                code,
                customerId: c.id,
    ...
    for (let i = 0; i < 50; i++) {
        ...
        await prisma.salesActivity.create({
            data: {
                ...
                dueAt: dueAt,
                status: actStatuses[Math.floor(Math.random() * actStatuses.length)],
                assignedToId: randomAssignee(),
                ...relId
            }
        });
