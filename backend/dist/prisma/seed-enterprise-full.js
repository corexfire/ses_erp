...
exports.seedEnterpriseFull = seedEnterpriseFull;
...
async function seedEnterpriseFull(prismaInstance, tenantId) {
    ...
    const arAccount = await prismaInstance.coaAccount.findFirst({ where: { tenantId, code: { startsWith: '1-12' } } });
    const revAccount = await prismaInstance.coaAccount.findFirst({ where: { tenantId, code: { startsWith: '4-' } } });
    ...
    if (execWO) {
        ...
        if (!ipQcExists) {
            await prismaInstance.inProcessQc.create({
                data: { tenantId, workOrderId: execWO.id, status: 'PASSED', qtyInspected: 100, qtyPassed: 98, qtyFailed: 2, notes: 'Mid-production QC during bottling. 2 units failed cap seal test.' }
            });
        }
    }
...
//# sourceMappingURL=seed-enterprise-full.js.map