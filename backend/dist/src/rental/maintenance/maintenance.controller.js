...
let RentalMaintenanceController = class RentalMaintenanceController {
    ...
    async findAll(req, search, status) {
        ...
        const summary = {
            scheduled: await this.prisma.rentalMaintenance.count({ where: { tenantId: user.tenantId, status: 'SCHEDULED' } }),
            inProgress: await this.prisma.rentalMaintenance.count({ where: { tenantId: user.tenantId, status: 'IN_PROGRESS' } }),
            completed: await this.prisma.rentalMaintenance.count({ where: { tenantId: user.tenantId, status: 'COMPLETED' } }),
    ...
    async create(req, body) {
        ...
        await this.audit.log(user.tenantId, user.id, 'CREATE', 'RentalMaintenance', record.id, null, record);
    ...
    async update(req, id, body) {
        ...
        await this.audit.log(user.tenantId, user.id, 'UPDATE', 'RentalMaintenance', id, existing, updated);
