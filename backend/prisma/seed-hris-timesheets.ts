...
export async function seedHrisTimesheets(prisma: PrismaClient, tenantId: string) {
  ...
  // Seed for April 2026 (Working days Mon-Fri)
  ...
  for (const emp of employees) {
    ...
    for (let d = 1; d <= daysInApril; d++) {
      ...
      // For each working day, create 1-3 entries totaling 8-10 hours
      const numTasks = Math.floor(Math.random() * 2) + 1; // 1 to 2 tasks per day
      let dailyHoursLeft = 8 + (Math.random() > 0.8 ? 2 : 0); // Some days have 10 hours (OT)
