...
async function main() {
  ...
  for (let i = 0; i < 40; i++) {
     ...
     // 50% chance to tie to a contract, 50% chance to tie strictly to empty internal assets
     if (Math.random() > 0.5 && contracts.length > 0) {
         const c = contracts[Math.floor(Math.random() * contracts.length)];
         contractId = c.id;
         assetId = c.assetId; // Maybe null or filled
     } else if (allAssets.length > 0) {
         assetId = allAssets[Math.floor(Math.random() * allAssets.length)].id;
     }

     const scheduledDate = new Date();
     scheduledDate.setDate(scheduledDate.getDate() - Math.floor(Math.random() * 60) + 10); // Between -50 days and +10 days
     ...
     if (status === 'COMPLETED' || status === 'IN_PROGRESS') {
         costAmount = (Math.floor(Math.random() * 95) + 5) * 100000; // Rp 500k to Rp 10M
     }

     const ticketNo = `RWO-2026-${String(1000 + i + 1).padStart(4, '0')}`;
     let resolutionNotes = null;
     if (status === 'COMPLETED') resolutionNotes = `Telah ditindaklanjuti. Aset berfungsi optimal kembali. Mekanik yang bertugas: ${issue.mechanicName}`;
