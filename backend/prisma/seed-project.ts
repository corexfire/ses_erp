...
async function main() {
  ...
  for (const project of createdProjects) {
    if (Math.random() > 0.8) continue; // 20% projects don't have WBS yet (e.g. DRAFT)
    ...
    for (const phase of phases) {
      ...
      if (project.status === 'COMPLETED' || project.status === 'CLOSED') {
        actualProgress = 100;
        actualCost = plannedCst * (0.9 + Math.random() * 0.2); // +/- 10% variance
        wbsStatus = 'COMPLETED';
      } else if (project.status === 'IN_PROGRESS') {
        if (phase.codeSuffix === 'P1') {
          actualProgress = 100; wbsStatus = 'COMPLETED'; actualCost = plannedCst;
