...
export async function seedInterCompany(prisma: PrismaClient, tenantId: string) {
  console.log('🏦 Seeding Integrated Inter-Company Transactions...');

  // 1. Fetch Dependencies (Branches)
  ...
  const apHq = await prisma.coaAccount.findFirst({ where: { tenantId, code: '2-1410-00' } }); // From SBY perspective, this is Hutang to HQ (if HQ pays for SBY)
  
  // 3. STORY 1: FUND TRANSFER (Approved)
  // HQ transfers 500M IDR to Surabaya for "Initial OPEX Branch Opening"
  ...
  // ACCOUNTING INTEGRATION for Story 1
  ...
  // 4. STORY 2: COST ALLOCATION (Pending)
  // HQ allocates HR Overhead Costs to Medan Branch
  ...
  // 5. STORY 3: INVENTORY TRANSFER (Draft)
  // Surabaya returns surplus stock to HQ
  ...
  // 6. STORY 4: PROFIT SHARE (Rejected)
