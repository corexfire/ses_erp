...
async function main() {
  ...
  for (const c of customers) {
      // Create user for 70% of customers
      ...
      if (portalUser.portalStatus === 'ACTIVE') {
          const logCount = Math.floor(Math.random() * 4) + 1; // 1 to 4 logs
          for (let i = 0; i < logCount; i++) {
              const types = ['LOGIN', 'INVOICE_VIEW', 'QUOTE_VIEW', 'PROFILE_UPDATE'];
              const type = types[Math.floor(Math.random() * types.length)];
              
              let desc = '';
              if (type === 'LOGIN') desc = 'Client berhasil masuk ke antarmuka B2B ERP.';
              else if (type === 'INVOICE_VIEW') desc = `Client mengunduh/melihat digital Invoice terkait transaksi bulan ini.`;
              else if (type === 'QUOTE_VIEW') desc = `Client mengeksplorasi penawaran/draft Sales Quotation yang diajukan sistem.`;
              else desc = 'Client melakukan sinkronisasi data NPWP / alamat tagihan.';
