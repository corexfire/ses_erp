...
export async function seedHrisShiftIntegrated(prisma: PrismaClient, tenantId: string) {
  console.log('🌱 Seeding Advanced Integrated HRIS Shift Data for FY 2026...');

  // 1. Master Shift Data (Expanding beyond standard)
  ...
  // 2. Identify Employees for Rotation (OPS, WMS, HSE)
  ...
  // 3. Update Attendance with Shift Rotation Labels (April 2026)
  for (const emp of rotatingEmployees) {
    ...
    for (let day = 1; day <= 30; day++) {
      ...
      // Week 1 (1-7): Morning
      // Week 2 (8-14): Afternoon
      // Week 3 (15-21): Night
      // Week 4 (22-30): Morning again
      let shiftCode = 'SH-MORNING';
      if (day > 7 && day <= 14) shiftCode = 'SH-AFTERNOON';
      else if (day > 14 && day <= 21) shiftCode = 'SH-NIGHT-ADV';
      else if (day > 21) shiftCode = 'SH-MORNING';
      ...
      if (existingAtnd) {
        // Tag the existing record with the specific shift used
        await prisma.attendance.update({
          where: { id: existingAtnd.id },
          data: { notes: `[Roster Shift: ${shiftCode}] ${existingAtnd.notes || ''}`.trim() }
        });

        // Adjust check-in/out times slightly to match the shift if they were generic
