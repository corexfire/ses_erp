...
export async function seedDailyProgress(prisma: PrismaClient, tenantId: string) {
  ...
  if (projects.length === 0) return console.log('⚠️  No projects found for seeding progress.');

  const weatherOptions = ['CERAH', 'BERAWAN', 'Hujan Ringan', 'HUJAN LEBAT'];
  const startDay = new Date();
  startDay.setDate(startDay.getDate() - 5); // Start 5 days ago

  for (let i = 0; i < projects.length; i++) {
    const proj = projects[i];
    
    // Generate 3 reports per project
    for (let d = 0; d < 3; d++) {
