...
export async function seedIntegratedErp(prisma: PrismaClient, tenantId: string) {
  console.log('--- SEEDING INTEGRATED ENTERPRISE TRANSACTIONS (GOLDEN THREAD) ---');

  // CLEANUP EXISTING INTEGRATED DATA TO ENSURE IDEMPOTENCY
  ...
  const purchaseInvoice = await prisma.purchaseInvoice.create({
    data: {
      ...
      code: pinvCode,
      supplierId: supplier.id,
      orderId: purchaseOrder.id, // Corrected from purchaseOrderId to orderId
