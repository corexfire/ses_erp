...
export async function seedFinanceBudget(prisma: PrismaClient, tenantId: string) {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    // IT - Monthly Bandwidth & Cloud (CC-ITD)
    if (centers.IT) {
      ...
      await prisma.budget.create({
        data: {
          ...
          fiscalYear,
          periodNo: m,
          accountCode: '6-2320-00', // Internet & Komunikasi
          costCenterId: centers.IT.id,
          amount: 25000000, // 25M per month
          spentAmount: m < 4 ? 24000000 + (Math.random() * 3000000) : 0, // Jan-Mar active
          status: m < 4 ? 'APPROVED' : 'DRAFT'
        }
      });
    }

    // MARKETING - Monthly Digital Ads (CC-MKT)
    if (centers.MKT) {
      ...
      await prisma.budget.create({
        data: {
          ...
          fiscalYear,
          periodNo: m,
          accountCode: '6-3100-00', // Iklan & Promosi Online
          costCenterId: centers.MKT.id,
          amount: 100000000, // 100M per month
