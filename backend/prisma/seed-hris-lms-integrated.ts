...
export async function seedHrisLmsIntegrated(prisma: PrismaClient, tenantId: string) {
  ...
  // 2. Fetch Employees for Enrollment
  const employees = await prisma.employee.findMany({ where: { tenantId } });

  // 3. Smart Enrollment & KPI Impact
  for (const emp of employees) {
    // A. Mandatory for everyone: ETH-200, ONB-001
    const universalMandatory = dbCourses.filter(c => ['ETH-200', 'ONB-001'].includes(c.code));
    
    // B. Mandatory for Operations/Project/HSE: HSE-101
    ...
    for (const crs of targetedCourses) {
      ...
      const score = isCompleted ? Math.floor(Math.random() * 20) + 81 : null; // 81-100 score
    ...
    // 4. KPI Boost - Link to performance if all mandatory are finished with high score
