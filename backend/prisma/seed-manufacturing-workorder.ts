...
async function main() {
  ...
  // ============================================================
  // WO 1: Kopi Susu Gula Aren - COMPLETED (3 months ago)
  // ============================================================
  const wo1StartDate = new Date(); wo1StartDate.setMonth(wo1StartDate.getMonth() - 3);
  const wo1EndDate = new Date(wo1StartDate); wo1EndDate.setDate(wo1EndDate.getDate() + 3);
  ...
  for (const op of wo1Ops) {
    ...
    if (!existing) {
      await prisma.workOrderOperation.create({
        data: { tenantId, workOrderId: wo1.id, ...op, startDate: wo1StartDate, endDate: wo1EndDate } as any
      });
    }
  }
  ...
  // ============================================================
  // WO 2: Syrup Gula Aren - COMPLETED last month
  // ============================================================
  const wo2StartDate = new Date(); wo2StartDate.setMonth(wo2StartDate.getMonth() - 1);
  const wo2EndDate = new Date(wo2StartDate); wo2EndDate.setDate(wo2EndDate.getDate() + 1);
  ...
  for (const op of wo2Ops) {
    ...
    if (!existing) {
      await prisma.workOrderOperation.create({
        data: { tenantId, workOrderId: wo2.id, ...op, startDate: wo2StartDate, endDate: wo2EndDate } as any
      });
    }
  }
  ...
  // ============================================================
  // WO 3: IN_PROGRESS - Currently running
  // ============================================================
  const wo3Start = new Date(); wo3Start.setDate(wo3Start.getDate() - 2);
  const wo3End = new Date(); wo3End.setDate(wo3End.getDate() + 1);
  ...
  for (const op of wo3Ops) {
    ...
    if (!existing) {
      await prisma.workOrderOperation.create({
        data: { tenantId, workOrderId: wo3.id, ...op, startDate: wo3Start } as any
      });
    }
  }
  ...
  // ============================================================
  // WO 4: RELEASED - Espresso sub-assembly
  // ============================================================
  const wo4Start = new Date(); wo4Start.setDate(wo4Start.getDate() + 1);
  const wo4End = new Date(); wo4End.setDate(wo4End.getDate() + 2);
  ...
  // ============================================================
  // WO 5: DRAFT - Future production plan
  // ============================================================
  const wo5Start = new Date(); wo5Start.setDate(wo5Start.getDate() + 7);
  const wo5End = new Date(); wo5End.setDate(wo5End.getDate() + 10);
