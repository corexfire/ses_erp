import { PrismaClient, WorkflowInstanceStatus } from './generated/client';

export async function seedWorkflowInstances(prisma: PrismaClient, tenantId: string) {
  ...
  const findDef = (key: string) => definitions.find(d => d.moduleKey === key || d.name.includes(key));

  // 2. Create Dummy PR Approval
  ...
  if (prDef && prDef.steps.length > 0) {
    ...
    await prisma.workflowInstance.create({
      data: {
        ...
        slaDueAt: new Date(Date.now() + 24 * 60 * 60 * 1000), // 24h from now
      }
    });
    console.log('✅ Seeded: Pending PR Approval for Admin');
  }

  // 3. Create Dummy SO Approval (Overdue)
  ...
  if (soDef && soDef.steps.length > 0) {
    ...
    await prisma.workflowInstance.create({
      data: {
        ...
        status: WorkflowInstanceStatus.PENDING,
        assignedToUserId: admin.id,
        slaDueAt: new Date(Date.now() - 48 * 60 * 60 * 1000), // 48h ago (Overdue)
      }
    });
    console.log('✅ Seeded: Overdue SO Approval for Admin');
  }

  // 4. Create Dummy Stock Opname Approval
  ...
  if (invDef && invDef.steps.length > 0) {
    ...
    await prisma.workflowInstance.create({
      data: {
        ...
        status: WorkflowInstanceStatus.PENDING,
        assignedToUserId: admin.id,
        slaDueAt: new Date(Date.now() + 12 * 60 * 60 * 1000), // 12h from now
      }
    });
    console.log('✅ Seeded: Stock Opname Approval for Admin');
  }

  // 5. Create a Delegated Instance
  // First, create a delegation from another user to Admin
    ...
    if (hrDef && hrDef.steps.length > 0) {
        ...
        await prisma.workflowInstance.create({
          data: {
            ...
            currentStepNo: 1,
            status: WorkflowInstanceStatus.PENDING,
            assignedToUserId: hrUser.id, // Original assignee, but Admin can see it via delegation
