...
async function main() {
  ...
  // 2. Clear previous forecasts to keep data fresh
  ...
  for (const item of items) {
      // Simulate AI skipping some obscure items (we predict 75% of catalog)
      ...
      // Add standard deviations to confidence
      const dev = Math.random() * 15;
      const confidenceScore = (98 - dev).toFixed(2);
      const recommendedQty = Math.floor(actualDemand * 1.15); // buffer stock 15%
