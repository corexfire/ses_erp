...
export async function seedPaymentRequests(prisma: PrismaClient, tenantId: string) {
  ...
  const supplierBca = await prisma.supplier.findFirst({ where: { tenantId, code: 'SES-VN-MSFT' } }); // Microsoft from previous seed
