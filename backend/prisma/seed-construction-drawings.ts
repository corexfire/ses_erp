...
export async function seedDrawings(prisma: PrismaClient, tenantId: string) {
  console.log('🏗️  Seeding Construction Drawings...');

  const projects = await prisma.project.findMany({ where: { tenantId } });
  if (projects.length === 0) return console.log('⚠️  No projects found for seeding drawings.');
  ...
  for (const d of drawData) {
    const proj = projects[0]; // Link to first project for simplicity
