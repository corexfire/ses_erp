...
export async function seedProjectBillingIntegrated(prisma: PrismaClient, tenantId: string) {
...
async function seedSingleBilling(prisma: PrismaClient, tenantId: string, project: any, data: any) {
    // 1. Progress Claim (if not DP)
    ...
    // 4. Journal Entry (for PAID status)
    if (data.status === 'PAID') {
