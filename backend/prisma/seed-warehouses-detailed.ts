import type { PrismaClient, WarehouseType, BinType } from './generated/client';

export async function seedWarehousesDetailed(prisma: PrismaClient, tenantId: string) {
  ...
  const dc = await prisma.warehouse.upsert({
    ...
    create: {
      ...
      capacityWeight: 5000000, // 5000 tons
      capacityVolume: 25000,   // 25000 m3
      isActive: true,
    }
  });
  ...
  for (const z of zones) {
    ...
    // Seed Bins for each zone
    const binTypeMap: Record<string, BinType> = {
      'Z-IN-01': 'RECEIVING',
      'Z-BK-01': 'STORAGE',
      'Z-PK-01': 'PICKING',
      'Z-SH-01': 'SHIPPING',
