...
async function main() {
  ...
  // 2. Clear old enrollments to prevent massive stacking if seeded repeatedly
  ...
  for (const emp of employees) {
      // Pick 2-4 random courses for this employee
      ...
      for (const crs of selectedCourses) {
          ...
          enrolledAt.setDate(enrolledAt.getDate() - (Math.floor(Math.random() * 90) + 1)); // Enrolled up to 90 days ago
