...
export async function seedTaxInvoiceInputIntegrated(prisma: PrismaClient, tenantId: string) {
  ...
  // 2. Suppliers Data with Tax Identity
  const suppliers = [
  ...
  for (const s of scenarios) {
    ...
    const purchaseInv = await prisma.purchaseInvoice.upsert({
      ...
      create: {
        ...
        invoiceDate: s.date,
        status: s.status as ProcurementDocStatus,
        subtotal: s.amount,
        taxAmount: s.vat,
    ...
    // C. Create Tax Invoice (Faktur Masukan)
    await prisma.taxInvoice.upsert({
      ...
      create: {
        ...
        invoiceDate: s.date,
        supplierId: s.supplier.id,
        customerName: s.supplier.name, // For INPUT invoices, customerName stores Vendor Name
  ...
  await prisma.taxInvoice.upsert({
      ...
      create: {
          ...
          invoiceType: 'INPUT',
          status: 'VERIFIED',
          fpNumber: importInvNo, // PIB often uses its own number
          fpDate: importDate,
          taxPeriod: '04',
          taxYear: 2026,
